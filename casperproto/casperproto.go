...
func (p *CasperServerClient) SendReplicationQuery(ctx context.Context, hash string, blockedIpfsAddr string, sizeToStore int64) (r string, err error) {
  var _args6 CasperServerSendReplicationQueryArgs
  _args6.Hash = hash
  _args6.BlockedIpfsAddr = blockedIpfsAddr
  _args6.SizeToStore = sizeToStore
  var _result7 CasperServerSendReplicationQueryResult
  if err = p.c.Call(ctx, "SendReplicationQuery", &_args6, &_result7); err != nil {
    return
  }
...
func (p *CasperServerClient) Ping(ctx context.Context) (r int64, err error) {
  var _args8 CasperServerPingArgs
  var _result9 CasperServerPingResult
  if err = p.c.Call(ctx, "Ping", &_args8, &_result9); err != nil {
    return
  }
...
func NewCasperServerProcessor(handler CasperServer) *CasperServerProcessor {
  ...
  self10 := &CasperServerProcessor{handler:handler, processorMap:make(map[string]thrift.TProcessorFunction)}
  self10.processorMap["SendUploadQuery"] = &casperServerProcessorSendUploadQuery{handler:handler}
  self10.processorMap["SendDownloadQuery"] = &casperServerProcessorSendDownloadQuery{handler:handler}
  self10.processorMap["SendDeleteQuery"] = &casperServerProcessorSendDeleteQuery{handler:handler}
  self10.processorMap["SendReplicationQuery"] = &casperServerProcessorSendReplicationQuery{handler:handler}
  self10.processorMap["Ping"] = &casperServerProcessorPing{handler:handler}
return self10
}
